...
class WeChat(object):
    ...
    def get_department_list(self):
        ...
        #print info.get('name', '').encode('utf-8')
    ...
    def get_user_list(self):
        ...
        department_list=self.get_department_list()
        dep_dict={}
        for i in department_list:
          dep_dict[i.get('id')]=i.get('name')   
        return [{
                'name': info.get('name', ''),
                'userid': info.get('userid', '').lower(),
                'department': info.get('department', [0])[0],
                'department_name': dep_dict.get(info.get('department', [0])[0]), 
                'mobile': info.get('mobile', '18888888888'),
                'position': info.get('position', 'undefined'),
                'email': info.get('email', 'undefined@yongdaxing.cn'),
